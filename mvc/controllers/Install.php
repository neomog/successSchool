...
class Install extends CI_Controller {
    ...
    */
    ...
    public function index()
    {
        ...
        //Check Index Path
        if (file_exists($this->config->index_path)) {
            $this->data['success'][]        = 'Index file is loaded';
            @chmod($this->config->index_path, FILE_WRITE_MODE);
            if (is_really_writable($this->config->index_path) === FALSE) {
                $this->data['errors'][]     = 'Index file is non-writable';
            } else {
                $this->data['success'][]    = 'Index file is writable';
            }
        } else {
            $this->data['errors'][]         = 'Index file is unloaded';
        }
